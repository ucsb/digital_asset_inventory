...
Module settings are at `/admin/config/accessibility/digital-asset-inventory`.

### Archive Settings

| Setting | Description | Default |
|---------|-------------|---------|
| Enable Archive Functionality | Show/hide all archive features | Disabled |
| Enable Manual Archive | Allow manual entries for web pages and external resources | Disabled |
| Allow Archive In Use | Allow archiving documents still referenced by content | Disabled |
| Show archived label on links | Display label on links to archived content | Enabled |
| Archived label text | Text shown on archived links (parentheses added automatically) | Archived |
| ADA Compliance Deadline | Date separating Legacy vs. General Archives | April 24, 2026 |

### Scanner Settings

| Setting | Description | Default |
|---------|-------------|---------|
| Time budget per request (seconds) | Maximum seconds per batch request. Setting this too high may cause scans to fail on hosting platforms with strict timeout limits. | 10 |
| Abandoned scan timeout (seconds) | How long before a scan with no heartbeat is considered abandoned. The scanner sends heartbeats during processing; this threshold detects crashes vs. slow processing. | 900 |
| Additional excluded directories | Custom directory names to skip during scanning (one per line). Applied to both `public://` and `private://` file systems. | (none) |

**Built-in excluded directories** are always applied and cannot be modified. They cover system-generated directories (image styles, aggregated CSS/JS, config exports), module-generated directories (media thumbnails, CTools, XML Sitemap), and site-specific directories (wordmark).
